...
namespace Quibble.Client.Pages.Quizzes
{
    public partial class HostQuiz : IAsyncDisposable
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            if (Quiz == null) throw new InvalidOperationException($"Parameter {nameof(Quiz)} cannot be null");
            ...
            await QuizHubConnection.RegisterForUpdatesAsync(Quiz.Id).ConfigureAwait(false);
            await RoundHubConnection.RegisterForUpdatesAsync(Quiz.Id).ConfigureAwait(false);
            await QuestionHubConnection.RegisterForUpdatesAsync(Quiz.Id).ConfigureAwait(false);
        }
